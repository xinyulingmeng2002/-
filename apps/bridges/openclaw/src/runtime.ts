...
export async function runOpenClawBridgeSession(
  ...
  const intervalHandle = setIntervalFn(() => {
    let currentSession: OpenClawBridgeSessionRecord;
    try {
      currentSession = readOpenClawBridgeSessionFile(options.sessionFilePath);
    } catch (error) {
...
export async function getOpenClawBridgeWorkspaceSnapshot<T = unknown>(
...
function isStructuredMentionForAgent(event: unknown, session: OpenClawBridgeSessionRecord): boolean {
...
function resolveAttentionTags(event: unknown, session: OpenClawBridgeSessionRecord, events: unknown[]): string[] {
...
function annotateWatchedBatch(batch: unknown, session: OpenClawBridgeSessionRecord): unknown {
  const items = (batch as { items?: unknown })?.items;
  ...
  while (!options.signal?.aborted) {
    ...
    if (hasBatchItems(batch)) {
      await options.onBatch?.(
        annotateWatchedBatch(batch, readOpenClawBridgeSessionFile(options.sessionFilePath))
      );
    }
